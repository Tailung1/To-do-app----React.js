    ...
    return (
        ...
        {todos.map((item)=>(
            ...
            className=" flex items-center gap-4 py-[20px] px-[24px]  " > 
              <input className="appearance-none w-6 h-6 rounded-full border-2 border-gray-300 checked:border-none checked:bg-[linear-gradient(to_bottom_right,#55DDFF,#C058F3)] flex items-center justify-center"
                onChange={(event)=>
                handleCheck(event,item.id)}
                type="checkbox"
              />
                {item.id === editId
                    ? <input value={editTodo} type='text' onChange={(event)=>setEditTodo(event.target.value)} />
                    : <span className={`${
                        item.status?
                         "line-through text-[#4D5067]"
                         : "text-[#C8CBE7]"
                        }  text-[20px] text-[]`} >{item.todo}
                      </span>
                }
